...
if (empty($_POST["otp"])) {
    ...
    if (empty($errors)) {
        $q = "INSERT INTO users (firstName, lastName, email, pass, otp, regDate, companyName) 
    VALUES ('$fn', '$ln', '$e', SHA2('$p', 256),$otp, NOW(), $c)";
        $r = @mysqli_query($link, $q);
        if ($r) {
            echo '<br>You are now registered.
				<a class="alert-link" href="index.html">Login</a>
			';
        }
